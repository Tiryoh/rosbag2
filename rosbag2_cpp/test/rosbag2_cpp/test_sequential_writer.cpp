  ...
  uint64_t expected_counter)
{
  ...
  static const std::regex timestamp_pattern(R"(\d{4}_\d{2}_\d{2}-\d{2}_\d{2}_\d{2})");
...
{
  std::regex pattern("_\\d{4}_\\d{2}_\\d{2}-\\d{2}_\\d{2}_\\d{2}$");
  std::smatch match;
  if (std::regex_search(filename, match, pattern)) {
...
TEST_F(SequentialWriterTest, writer_splits_when_storage_bagfile_size_gt_max_bagfile_size) {
  ...
  // Record timestamp before writer reset (file creation happens during write/split)
...
TEST_F(SequentialWriterTest, filename_extracts_prefix_from_timestamped_directory_name)
{
  ...
  // Record end timestamp after file creation, for tolerance-based comparison with filename timestamp
  auto recorded_time = std::chrono::system_clock::now();
  writer_.reset();
