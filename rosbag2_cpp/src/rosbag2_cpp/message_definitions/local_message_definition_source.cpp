  ...
  const DefinitionIdentifier & definition_identifier)
{
  ...
  auto resource = ament_index_cpp::get_resource("rosidl_interfaces", package_name);
  if (resource.resourcePath.has_value()) {
    share_dir_path = resource.resourcePath.value() / "share" / package_name;
    ROSBAG2_CPP_LOG_DEBUG(
      "resource_content : \n%s for package: '%s' ,\n share_dir: '%s'\n, topic_type: '%s'",
      resource.contents.c_str(), package_name.c_str(), share_dir_path.c_str(),
      topic_type.c_str());
  } else {
    ROSBAG2_CPP_LOG_DEBUG(
  ...
  std::stringstream ss(resource.contents);
  std::string line;
